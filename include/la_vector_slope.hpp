...
int32_t safe_max_slope = 0;

template<typename K, uint8_t t_bpc = 0, uint8_t S = 0>
class la_vector_slope {
...
public:
    la_vector_slope() = default;

    explicit la_vector_slope(std::vector<K> &data) : la_vector_slope(data.begin(), data.end()) {};

    template<class RandomIt>
    la_vector_slope(RandomIt begin, RandomIt end)
            : front(*begin),
              back(*std::prev(end)),
...
template<typename K, uint8_t t_bpc, uint8_t S>
struct la_vector_slope<K, t_bpc, S>::constant_bpc {
...
template<typename K, uint8_t t_bpc, uint8_t S>
struct la_vector_slope<K, t_bpc, S>::variable_bpc {
    uint8_t bpc;
...
template<typename K, uint8_t t_bpc, uint8_t S>
struct la_vector_slope<K, t_bpc, S>::segment : base_segment_type {
    position_type first;
    ...
    segment(const canonical_segment &cs, uint8_t bpc, position_type corrections_offset,
        ...
        auto[cs_slope, cs_intercept] = cs.template get_fixed_point_segment<S>(cs.get_first_x());
        ...
        if (this->bpc != 0) {
            for (auto k = i; k < j; k++) {
                ...
                if (BIT_WIDTH(correction) > this->bpc) {
                    safe_max_slope++;
                    auto[cs_slope, cs_intercept] = cs.template get_fixed_point_segment<0>(cs.get_first_x());
                    first = cs.get_first_x();
                    intercept = cs_intercept;
                    slope = cs_slope;
                    break;
...
template<typename K, uint8_t t_bpc, uint8_t S>
class la_vector_slope<K, t_bpc, S>::la_iterator {
    using parent_type = const la_vector_slope<K, t_bpc, S>;
