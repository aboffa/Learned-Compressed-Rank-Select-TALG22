...
protected:
    D ds;
    typename D::select_1_type select_support;
    typename D::rank_1_type rank_support;
    size_t n;
...
public:
    ...
    inline double bits_per_element() const {
        double size = (double) (sdsl::size_in_bytes(ds) * 8);
        size += (double) (sdsl::size_in_bytes(select_support) * 8);
        size += (double) (sdsl::size_in_bytes(rank_support) * 8);
        size /= static_cast<double>(n);
        return size;
    };

    inline size_t rank(T x) const {
        return rank_support.rank(x);
    }

    inline void decode(T *out) const {
        for (size_t i = 0; i < n; ++i) {
            *(out++) = select_support(i + 1);
        }
    }
