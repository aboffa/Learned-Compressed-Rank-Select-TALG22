...
#include "slope_utils.h"
#include "slope_piecewise_linear_model.hpp"
...
public:
    ...
    using canonical_segment = typename Slope_OptimalPiecewiseLinearModel<position_type, K>::CanonicalSegment;
    ...
    static std::vector<canonical_segment> standard_segmentation(RandomIt begin, RandomIt end, uint8_t bpc) {
        ...
        slope_make_segmentation_par(n, eps, in_fun, out_fun);
        return out;
    }
...
public:
    la_vector_bs() = default;

    explicit la_vector_bs(std::vector<K> &data) : la_vector_bs(data.begin(), data.end()) {};

    template<class RandomIt>
    la_vector_bs(RandomIt begin, RandomIt end)
            : front(*begin),
              back(*std::prev(end)),
...
struct la_vector_bs<K, t_bpc>::constant_bpc {
...
template<typename K, uint8_t t_bpc>
struct la_vector_bs<K, t_bpc>::variable_bpc {
    uint8_t bpc;
...
template<typename K, uint8_t t_bpc>
struct la_vector_bs<K, t_bpc>::segment : base_segment_type {
    ...
    segment(const canonical_segment &cs, uint8_t bpc, position_type corrections_offset,
        ...
        std::vector<std::tuple<long double, long double>> segments_slopes(9);
        std::vector<index_entropy> entropies(9);
        {
            segments_slopes[0] = cs.template get_fixed_point_segment<0>(cs.get_first_x());
            segments_slopes[1] = cs.template get_fixed_point_segment<1>(cs.get_first_x());
            segments_slopes[2] = cs.template get_fixed_point_segment<2>(cs.get_first_x());
            segments_slopes[3] = cs.template get_fixed_point_segment<3>(cs.get_first_x());
            segments_slopes[4] = cs.template get_fixed_point_segment<4>(cs.get_first_x());
            segments_slopes[5] = cs.template get_fixed_point_segment<5>(cs.get_first_x());
            segments_slopes[6] = cs.template get_fixed_point_segment<6>(cs.get_first_x());
            segments_slopes[7] = cs.template get_fixed_point_segment<7>(cs.get_first_x());
            segments_slopes[8] = cs.template get_fixed_point_segment<8>(cs.get_first_x());
            ...
            for (auto h = 0; h < 9; ++h) {
                auto _intercept = std::get<1>(segments_slopes[h]);
                auto _slope = std::get<0>(segments_slopes[h]);

                for (auto k = i; k < j; k++) {
                    auto error = data[k] - (((_slope * (k - first))) + _intercept);
                    eoc.add(error);
                }
                long double _entropy = 0;
                std::map<int, int> occurrences(eoc.get());
                for (auto &occurrence : occurrences) {
                    _entropy += -(((long double) occurrence.second / (long double) (j - i)) *
                                  std::log2((long double) occurrence.second / (long double) (j - i)));
                    assert(!std::isnan(_entropy));
                    assert(_entropy >= 0);
                }
                entropies[h].index = h;
                entropies[h].entropy = _entropy;
                entropies[h].slope = _slope;
                entropies[h].intercept = _intercept;
            }
        }
        std::sort(entropies.begin(), entropies.end(), [](auto a, auto b){ return a.entropy < b.entropy;});

        if (this->bpc != 0) {
            size_t h = 0;
            bool approximation_error = true;
            // in the worst case, slope 0 (MMAX) is seleted
            while(approximation_error) {
                assert(h <= 9);
                approximation_error = false;
                slope = entropies[h].slope;
                intercept = entropies[h].intercept;
                for (auto k = i; k < j; k++) {
                        auto error = data[k] - approximate(k);
                        auto correction = uint64_t(error + epsilon);
                        if (BIT_WIDTH(correction) > this->bpc) {
                            // approximation error with this slope
                            approximation_error = true;
                            break;
                        }
                    }
                h++;
    ...
    larger_signed_key_type approximate(size_t i) const {
        return (((slope) * (i - first))) + intercept;
    }

    std::pair<position_type, position_type> approximate_position(const K &value) const {
        long double p = (((long double) (value) - intercept) * slope) + first;
        long double epsilon = (long double)(BPC_TO_EPSILON(this->bpc));
        long double bound = 1 + (epsilon * slope);
        return {std::max<long double>(0, p), bound};
    }
...
template<typename K, uint8_t t_bpc>
class la_vector_bs<K, t_bpc>::la_iterator {
    using parent_type = const la_vector_bs<K, t_bpc>;
