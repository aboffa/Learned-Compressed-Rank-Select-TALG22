...
const bool write_partitions = false;

namespace ds2i {

    // hybrid means the type of hybridness
    // 0 -> no hybrid, just Elias-Fano
    // 1 -> original hybrid (Elias-Fano + RCBV + Run Length)
    // 2 -> LA-vector hybrid (Elias-Fano + RCBV + Run Length + Segments)
    ...
    struct partitioned_sequence {
        ...
        static std::tuple<uint64_t, uint64_t, uint64_t, uint64_t, uint64_t, uint64_t, uint64_t, uint64_t, uint64_t>
        write(succinct::bit_vector_builder &bvb,
            ...
            /*auto cost_fun = [&](uint64_t universe, uint64_t n) {
                return base_sequence_type::template bitsize<hybrid>(params, universe, n) + conf.fix_cost;
            };*/

            optimal_partition<hybrid> opt(begin, universe, n, params, conf.eps1, conf.eps2);
            std::ofstream myfile;
            if constexpr(write_partitions) {
                myfile.open("/home/boffa/new_my_benchmark/results/segments_start_end_type/dataset_with_n_u_hyb_" +
                            std::to_string(n) + "_" + std::to_string(universe) + "_" +
                            std::to_string(hybrid) + ".csv");
                myfile << "type_partition,start_partition,end_partition\n";
            }

            uint64_t prev_endpoint = 0;
            uint64_t partitions_ef = 0;
            uint64_t partitions_rcbv = 0;
            uint64_t partitions_allones = 0;
            uint64_t partitions_segment = 0;

            uint64_t n_ef = 0;
            uint64_t n_rcbv = 0;
            uint64_t n_allones = 0;
            uint64_t n_segment = 0;
            ...
            bitsize += 2 * ceil_log2(partitions) + 1;
            ...
            if (partitions == 1) {
                ...
                if (n > 1) {
                    if (cur_base + cur_partition.back() + 1 == universe) {
                        ...
                        bitsize += ceil_log2(ceil_log2(cur_partition.back())) + ceil_log2(cur_partition.back()) + 1;
                    }
                }

                auto type_size = opt.cost_fun(cur_partition.back() + 1, cur_partition.size(), 0, params);
                bitsize += (std::get<1>(type_size)  - conf.fix_cost);
                if (std::get<0>(type_size) == elias_fano) {
                    partitions_ef++;
                    if constexpr(write_partitions)
                        myfile << "EF,";
                    n_ef += cur_partition.size();
                }
                if (std::get<0>(type_size) == ranked_bitvector){
                    partitions_rcbv++;
                    if constexpr(write_partitions)
                        myfile << "BV,";
                    n_rcbv += cur_partition.size();
                }
                if (std::get<0>(type_size) == all_ones){
                    partitions_allones++;
                    if constexpr(write_partitions)
                        myfile << "RL,";
                    n_allones += cur_partition.size();
                }
                if (std::get<0>(type_size) == segment) {
                    partitions_segment++;
                    if constexpr(write_partitions)
                        myfile << "SE,";
                    n_segment += cur_partition.size();
                }
                if constexpr(write_partitions)
                    myfile << prev_endpoint << "," << prev_endpoint + cur_partition.size() - 1 << std::endl;
                base_sequence_type::write(bvb, cur_partition.begin(),
                                          cur_partition.back() + 1,
                                          cur_partition.size(),
                                          params, std::get<2>(type_size));
            } else {
                ...
                for (size_t p = 0; p < opt.partition.size(); ++p) {
                    // starting index current partition
                    size_t prev = (p == 0) ? 0 : opt.partition[p - 1];
                    cur_partition.clear();
                    uint64_t value = 0;
                    ...
                    assert(!cur_partition.empty());
                    auto type_size = opt.cost_fun(cur_partition.back() + 1, cur_partition.size(), prev, params);
                    bitsize += (std::get<1>(type_size) - conf.fix_cost);
                    if (std::get<0>(type_size) == elias_fano) {
                        partitions_ef++;
                        if constexpr(write_partitions)
                    ...
                    prev_endpoint += cur_partition.size();
                ...
                bitsize += 2 * ceil_log2(endpoint_bits) + 1;

                bvb.append(bv_sizes);
                bitsize += bv_sizes.size();
                bvb.append(bv_upper_bounds);
                bitsize += bv_upper_bounds.size();

                for (uint64_t p = 0; p < endpoints.size() - 1; ++p) {
                    bvb.append_bits(endpoints[p], endpoint_bits);
                    bitsize += endpoint_bits;
                }
            ...
            if constexpr(write_partitions)
                myfile.close();
            return {bitsize, n_ef, partitions_ef, n_rcbv, partitions_rcbv, n_allones, partitions_allones,
                    n_segment, partitions_segment};
        }
