...
namespace ds2i {
    ...
    struct uniform_partitioned_sequence {
        ...
        static std::tuple<uint64_t, uint64_t, uint64_t, uint64_t> write(succinct::bit_vector_builder &bvb,
                                                                        Iterator begin,
            ...
            if (partitions == 1) {
                ...
                std::pair<index_type, uint64_t> type_size = base_sequence_type::bitsize_type(params, universe, n);
                if (type_size.first == elias_fano)
                    partitions_ef++;
                if (type_size.first == ranked_bitvector)
                    partitions_rcbv++;
                if (type_size.first == all_ones)
                    partitions_allones++;
                bitsize += type_size.second;

                assert(std::is_sorted(cur_partition.begin(), cur_partition.end()));
                ...
                for (size_t p = 0; p < partitions; ++p) {
                    ...
                    for (; cur_i < ((p + 1) * partition_size) && cur_i < n; ++cur_i, ++it) {
                        value = *it;
                        assert(cur_partition.empty() || cur_partition.back() <= (value - cur_base));
                    ...
                    std::pair<index_type, uint64_t> type_size = base_sequence_type::bitsize_type(params,
                                                                                                 cur_partition.back() + 1,
                                                                                                 cur_partition.size());
                    if (type_size.first == elias_fano)
                        partitions_ef++;
            ...
            return {bitsize, partitions_ef, partitions_rcbv, partitions_allones};
        }
