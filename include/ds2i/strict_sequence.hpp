...
namespace ds2i {
    ...
    struct strict_sequence {
        ...
        bitsize_type(global_parameters const &params, uint64_t universe, uint64_t n) {
            uint64_t best_cost = all_ones_sequence::bitsize(params, universe, n);
            index_type best_index_type = all_ones;
            ...
            if (ef_cost < best_cost) {
                best_cost = ef_cost;
                best_index_type = elias_fano;
            }

            uint64_t rb_cost = compact_ranked_bitvector::bitsize(sparams, universe, n) + type_bits;
            if (rb_cost < best_cost) {
                best_cost = rb_cost;
                best_index_type = ranked_bitvector;
            }

            return std::pair<index_type, uint64_t>(best_index_type, best_cost);
        }
